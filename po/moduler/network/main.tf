...
resource "azurerm_subnet" "subnet_02" {
	...
	resource_group_name = var.ressurs_gruppe
	virtual_network_name = azurerm_virtual_network.vnetwork_01.name
	address_prefixes = [var.subnet_range[1]]
}
...
resource "azurerm_subnet" "subnet_03" {
	...
	resource_group_name = var.ressurs_gruppe
	virtual_network_name = azurerm_virtual_network.vnetwork_01.name
	address_prefixes = [var.subnet_range[2]]
}

resource "azurerm_subnet" "subnet_04" {
	name = var.subnet[3]
	resource_group_name = var.ressurs_gruppe
	virtual_network_name = azurerm_virtual_network.vnetwork_01.name
	address_prefixes = [var.subnet_range[3]]
}

resource "azurerm_subnet" "subnet_05" {
	name = var.subnet[4]
	resource_group_name = var.ressurs_gruppe
	virtual_network_name = azurerm_virtual_network.vnetwork_01.name
	address_prefixes = [var.subnet_range[4]]
}

resource "azurerm_subnet" "subnet_06" {
	name = var.subnet[5]
	resource_group_name = var.ressurs_gruppe
	virtual_network_name = azurerm_virtual_network.vnetwork_01.name
	address_prefixes = [var.subnet_range[5]]
}
